...
# Whitelist of columns we will send to Supabase (everything else is dropped)
SAFE_COLUMNS = [
    "surrogate_key",
    "header",
    "location",
...
NUMERIC_COLUMNS = ["price", "cashFlow", "ebitda", "initialFee", "initialCapital"]
...
class DatabaseManager:
    ...
    def _transform_for_table(self, listing: Dict[str, Any]) -> Dict[str, Any]:
        """
        Produce a SAFE, MINIMAL row matching your table, with only whitelisted columns.
        All BIGINTs are coerced to integers; everything else is None or text.
        """
        ...
        safe = {
            ...
            "urlStub": url_stub or None,
            "price": _to_bigint(listing.get("price")),
            "cashFlow": _to_bigint(listing.get("cashFlow")),
            "ebitda": _to_bigint(listing.get("ebitda")),
            "initialFee": _to_bigint(listing.get("initialFee")),
            "initialCapital": _to_bigint(listing.get("initialCapital")),
    ...
    def upsert_listings(self, listings: List[Dict[str, Any]]) -> bool:
        ...
        # Filter to cleaning only, then transform to SAFE rows
        ...
        # Final belt & suspenders: rebuild rows strictly from SAFE_COLUMNS and coerce numerics
        safe_rows: List[Dict[str, Any]] = []
        for row in transformed:
            safe = {k: row.get(k) for k in SAFE_COLUMNS}
            for k in NUMERIC_COLUMNS:
                safe[k] = _to_bigint(safe.get(k))
            safe_rows.append(safe)

        # Optional: log one sample row’s keys and numeric values for sanity
        if safe_rows:
            logger.info(f"Safe payload keys: {list(safe_rows[0].keys())}")
            logger.info(
                f"Sample numerics: price={safe_rows[0]['price']} "
                f"cashFlow={safe_rows[0]['cashFlow']} ebitda={safe_rows[0]['ebitda']} "
                f"initialFee={safe_rows[0]['initialFee']} initialCapital={safe_rows[0]['initialCapital']}"
            )
    ...
    def scrape_listings(self, max_pages=100, workers=10):
        ...
        payload_template = {
            "bfsSearchCriteria": {
                ...
                "grossIncomeMin": 0,
                "grossIncomeMax": 0,
                "daysListedAgo": 30,  # wider window while testing
