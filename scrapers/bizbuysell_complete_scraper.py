"""
BizBuySell Complete Scraper - Production Ready
Based on jeffsosville/bbs-listings-scraper
Enhanced with:
...
class BizBuySellScraper:
    def __init__(self, vertical_slug='cleaning'):
    ...
    def scrape_state(self, state_code, max_pages=100, workers=10):
        """Scrape all listings from a specific state"""
        print(f"{Fore.CYAN}[*] Scraping {state_code} with {workers} workers...")

        if not self.token:
            print(f"{Fore.RED}[-] No authentication token available.")
            return []
        ...
        print(f"{Fore.GREEN}[+] {state_code}: Scraped {len(all_listings)} unique listings")
        return all_listings

    def scrape_all_listings(self, max_pages=100, workers=10):
        """Scrape all listings (no state filter) - original behavior"""
        print(f"{Fore.CYAN}[*] Scraping ALL listings with {workers} workers...")
        ...
        payload_template = {
            "bfsSearchCriteria": {
                ...
                "languageId": 10,
                "categories": None,
                "locations": None,  # No state filter
                "excludeLocations": None,
                "askingPriceMax": 0,
                "askingPriceMin": 0,
                "pageNumber": 1,
        ...
        def fetch_page(page_number):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"{Fore.RED}[-] Page {page_number}: Status {response.status_code}")
            except Exception as e:
                print(f"{Fore.RED}[-] Page {page_number}: {str(e)}")
            return []
        ...
        print(f"{Fore.GREEN}[+] Scraping complete! Total unique listings: {len(all_listings)}")
        return all_listings
    ...
    def save_to_postgres(self, listings):
        ...
        print(f"{Fore.CYAN}[*] Saving {len(listings)} listings to database...")
    ...
    def run(self, mode='all', states=None, max_pages=500, workers=10):
        """
        Main execution
        mode: 'all' = scrape all listings, 'states' = scrape by state
        states: list of state codes (e.g., ['CA', 'NY', 'TX'])
        """
        print(f"{Fore.CYAN}{'='*70}")
        print(f"{Fore.CYAN}BizBuySell Complete Scraper - {self.vertical_config['name']}")
        ...
        try:
            if mode == 'states':
                # Scrape by state
                states_to_scrape = states if states else US_STATES
                for state in states_to_scrape:
                    raw_listings = self.scrape_state(state, max_pages, workers)
                    ...
                    print(f"{Fore.GREEN}[+] {state}: Matched {len(filtered)} {self.vertical_config['name']}")

            else:  # mode == 'all'
                # Scrape all listings (no state filter)
                raw_listings = self.scrape_all_listings(max_pages, workers)
                self.stats['total_found'] = len(raw_listings)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="BizBuySell Complete Scraper")
    parser.add_argument('--vertical', type=str, choices=['cleaning', 'landscape', 'hvac'],
                        default='cleaning', help='Vertical to scrape')
    parser.add_argument('--mode', type=str, choices=['all', 'states'],
                        default='all', help='Scraping mode: all listings or by state')
    ...
    scraper = BizBuySellScraper(vertical_slug=args.vertical)
    scraper.run(mode=args.mode, states=states, max_pages=args.max_pages, workers=args.workers)
