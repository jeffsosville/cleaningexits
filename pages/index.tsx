...
export async function getServerSideProps() {
  // First try the dedicated view that already ranks/filters Top 10.
  let data: any[] | null = null;
  let error: any = null;

  // Preferred: price-first Top 10 view (you created/updated this)
  ({ data, error } = await supabase
    .from("top10_listings")
    .select(
      // View columns: header/notes/url already aliased in SQL
      "id, header, city, state, price, revenue, cash_flow, notes, url, image_url, scraped_at"
    )
    .limit(10));

  // Fallback: query the raw table if the view isn't available
  if (error || !data) {
    const DAYS_90_MS = 90 * 24 * 60 * 60 * 1000;
    const days90agoISO = new Date(Date.now() - DAYS_90_MS).toISOString();

    const includeOr =
      "title.ilike.%cleaning%,title.ilike.%janitorial%,title.ilike.%maid%,title.ilike.%carpet%,title.ilike.%window%,title.ilike.%pressure wash%";
    ...
    // Prefer higher asking prices, then revenue/cashflow, then recency
    const res = await q
    ...
    // Normalize shape to match the view’s column names
    if (data) {
      data = data.map((r: any) => ({
        id: r.id,
        header: r.title,
  ...
  const top10: Top10[] = (data ?? []).map((r: any) => ({
    id: String(r.id),
